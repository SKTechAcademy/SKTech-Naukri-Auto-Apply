...
test('only skips confirmed applications', () => { const url = 'https://example.test/job/1'; assert.equal(shouldSkip([{ url, status: 'DRY_RUN_MATCH' }], url), false); assert.equal(shouldSkip([{ url, status: 'APPLIED' }], url), true); });
